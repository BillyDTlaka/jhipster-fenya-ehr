...
import com.fenyatlala.ehr.config.KafkaSseConsumer;
import com.fenyatlala.ehr.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class FenyaTlalaEhrKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/fenya-tlala-ehr-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/fenya-tlala-ehr-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/fenya-tlala-ehr-kafka/unregister"));
                return;
            }
        }
